...
const registerSchema = z
  .object({
    ...
    phoneNumber: z.string().min(1, 'Phone number is required').regex(/^\+?[1-9]\d{6,14}$/, 'Enter a valid phone number'),
    email: z.string().email('Enter a valid email').optional().or(z.literal('')),
    password: z.string().min(8, 'Password must be at least 8 characters').regex(/(?=.*[a-z])(?=.*[A-Z])(?=.*\d)/, 'Must include uppercase, lowercase, and a number'),
  ...
  return (
    <Card className="w-full">
      <CardHeader className="items-center text-center">
        ...
        <CardTitle>Create your {meta.label} account</CardTitle>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <CardContent className="space-y-4">
          ...
          <FormField label="Full Name" error={errors.fullName?.message}>
            <Input type="text" placeholder="John Doe" {...register('fullName')} error={!!errors.fullName} />
          </FormField>
          <FormField label="Phone Number" error={errors.phoneNumber?.message}>
            <Input type="tel" placeholder="+251911234567" {...register('phoneNumber')} error={!!errors.phoneNumber} />
          </FormField>
          ...
          <FormField label="Password" error={errors.password?.message}>
            <div className="relative">
              <Input type={showPassword ? 'text' : 'password'} placeholder="SecurePass123" {...register('password')} error={!!errors.password} />
              <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-stone-400 hover:text-stone-600" tabIndex={-1}>
          ...
          <FormField label="Confirm Password" error={errors.confirmPassword?.message}>
            <div className="relative">
              <Input type={showConfirm ? 'text' : 'password'} placeholder="Repeat your password" {...register('confirmPassword')} error={!!errors.confirmPassword} />
              <button type="button" onClick={() => setShowConfirm(!showConfirm)} className="absolute right-3 top-1/2 -translate-y-1/2 text-stone-400 hover:text-stone-600" tabIndex={-1}>
